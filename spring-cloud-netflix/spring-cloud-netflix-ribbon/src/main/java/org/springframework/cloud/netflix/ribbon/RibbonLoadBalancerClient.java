...
public class RibbonLoadBalancerClient implements LoadBalancerClient {
	...
	public <T> T execute(String serviceId, LoadBalancerRequest<T> request, Object hint)
			throws IOException {
		// 根据服务名称去拿一个负载均衡器，如果没有就创建
		// 这边一个微服务名字，对应一个负载均衡器
		// 并且不同的负载均衡器,对应不同的spring context对象。为了隔离
		// 这边是通过父子容器实现的，
		// 至于为什么不直接都在一个applicationContext环境中配置？
		// 因为spring环境中不支持定义多个同一类型的Bean
		// 就算支持，也没法区分那个IRule Bean对应那个微服务

		ILoadBalancer loadBalancer = getLoadBalancer(serviceId);
		// 通过指定的负载均衡策略，返回一个服务地址
